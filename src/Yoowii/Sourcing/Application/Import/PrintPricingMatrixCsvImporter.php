...
final class PrintPricingMatrixCsvImporter
{
    ...
    public function import(
        ...
        string $csv,
    ): PrintPricingMatrixImportResult {
        ...
        try {
            ...
            $headerIndexes = $this->validateHeader($header, $definition->csvHeaders());
            $entries = [];
            $errors = [];
            $rowNumber = 1;
            $dataRows = 0;
            ...
            while (false !== ($row = fgetcsv($stream, null, $delimiter, '"', ''))) {
                ...
                try {
                    ...
                    foreach ($definition->pricingAxes() as $pricingAxis) {
                        $configurationValues[$pricingAxis] = $this->cell($row, $headerIndexes, $pricingAxis);
                    }

                    $configuration = $definition->configure($configurationValues);
            ...
            $matrix = new SupplierPricingMatrixVersion(
                ...
                [
                    ...
                    'product_schema_version' => $definition->schemaVersion(),
            ...
            return new PrintPricingMatrixImportResult($matrix, $importedRows);
        } finally {
            fclose($stream);
        }
    }
     ...
     */
    private function validateHeader(array $header, array $expectedHeaders): array
    {
        ...
        foreach ($normalizedHeader as $column) {
            if (!in_array($column, $expectedHeaders, true)) {
    ...
    {
        return trim((string) $row[$headerIndexes[$column]]);
    }
    ...
    private function nonNegativeInteger(array $row, array $headerIndexes, string $column): int
    {
        ...
        if (1 !== preg_match('/^\d+$/D', $rawValue)) {
            throw new \InvalidArgumentException(sprintf(
                'column "%s" must contain a non-negative integer amount, received "%s".',
                $column,
                $rawValue,
            ));
        }
