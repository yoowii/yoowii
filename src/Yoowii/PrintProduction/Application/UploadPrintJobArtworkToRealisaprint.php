...
final readonly class UploadPrintJobArtworkToRealisaprint
{
    ...
    public function __invoke(PrintJob $job): PrintJobSupplierFileTransfer
    {
        ...
        if (!$submission instanceof PrintJobSupplierSubmission || 'submitted' !== $submission->status()) {
            throw new \DomainException('A confirmed Realisaprint order is required before artwork upload.');
        }
        ...
        if (!$artwork instanceof PrintAsset) {
            throw new \DomainException('No active customer artwork is available.');
        }
        ...
        if (!is_array($files) || 1 !== count($files)) {
            throw new \DomainException('This Realisaprint order requires manual artwork placement: the API expects more than one file or returned no file instruction.');
        }
        $instruction = reset($files);
        if (!is_array($instruction) || !isset($instruction['folder'], $instruction['ext']) || !is_string($instruction['folder']) || !is_string($instruction['ext'])) {
            throw new \DomainException('The Realisaprint file instruction is malformed.');
        }
        ...
        if (!in_array($extension, $accepted, true)) {
            throw new \DomainException('The customer artwork extension is not accepted by Realisaprint for this configuration.');
        }
        ...
        if (!$transfer instanceof PrintJobSupplierFileTransfer) {
            $transfer = new PrintJobSupplierFileTransfer($submission, $artwork, $remotePath, new \DateTimeImmutable(), new \DateTimeImmutable());
            $this->entityManager->persist($transfer);
        }
        if ('uploaded' === $transfer->status()) {
            return $transfer;
        }
        ...
        try {
            if ($this->ftp->isEnabled()) {
                ...
                try {
                    $this->ftp->upload($remotePath, $stream);
                } finally {
                    fclose($stream);
                }
            }
            ...
            $transfer->markFailed($exception->getMessage(), $at);
        }
