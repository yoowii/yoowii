...
final readonly class RealisaprintFtpTransport
{
    ...
    {
        if (!function_exists('ftp_connect')) { throw new \RuntimeException('The PHP FTP extension is required for Realisaprint artwork upload.'); }
        $connection = ftp_connect($this->host, $this->port, 20);
        if (false === $connection || !ftp_login($connection, $this->user, $this->password)) { throw new \RuntimeException('Unable to connect to the Realisaprint FTP server.'); }
        try {
            ftp_pasv($connection, true);
            $destination = trim($this->baseDirectory . '/' . ltrim($remotePath, '/'), '/');
            $this->createDirectories($connection, dirname($destination));
            if (!ftp_fput($connection, $destination, $stream, \FTP_BINARY)) { throw new \RuntimeException('Unable to upload the artwork to Realisaprint FTP.'); }
        } finally { ftp_close($connection); }
    }
    private function createDirectories(mixed $connection, string $directory): void
    {
        $path = '';
